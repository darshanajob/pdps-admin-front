...
import {Agent_name,Agent_call_status, MobileNoS, Payment_status, S_half, Student_name, Student_nic} from "./agentReportCol";
const callStatusAgentReport = ({ dataColors }) => {

    //meta title
    document.title = "Call Status Report By Agent";
    ...
    useEffect(() => {
        ReportService.sortAllCallReportsByagent().then(res => {
    ...
    console.log(agents, 'from file');
    const CALL_NOT_YET = agents.filter(item => item.agent_call_status === "CALL_NOT_YET").length;
    const CALL_NOT_WORK = agents.filter(item => item.agent_call_status === "CALL_NOT_WORK").length;
    const CALL_NUMBER_BUSY = agents.filter(item => item.agent_call_status === "CALL_NUMBER_BUSY").length;
    const CALL_NOT_ANSWER = agents.filter(item => item.agent_call_status === "CALL_NOT_ANSWER").length;
    const CALL_ANSWER_SUCCESS = agents.filter(item => item.agent_call_status === "CALL_ANSWER_SUCCESS").length;
    ...
    const submitAddress = (e) => {
        ...
        ReportService.sortAllCallReportsByagent(data).then(res => {
            console.log('res', res);
            setAgents(res.data.students);
            /*console.log(res.data.students, 'from sercice');
            setAgents(res.data.students)  /
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                filterable: true,
                Cell: cellProps => {
                    return <Student_name {...cellProps} />;
                },
            },
            {
                Header: "Student NIC",
                accessor: "student_nic",
                filterable: true,
                Cell: cellProps => {
                    return <Student_nic {...cellProps} />;
                },
            },
            {
                ...
                filterable: true,
                Cell: cellProps => {
                    return <MobileNoS {...cellProps} />;
                },
            },
            {
                Header: "Call Status",
                accessor: "agent_call_status",
                filterable: true,
                Cell: cellProps => {
                    return (
                        <>
                            {" "}
                            <Agent_call_status {...cellProps} />{" "}
                        </>
                    );
                },
            },
    ...
    return (
        <React.Fragment>
            ...
            <div className="page-content">
                <Container fluid>
                    {/* table body */}
                    <Breadcrumbs title="Reports" breadcrumbItem="Call Status Report By Agent" />
...
export default withRouter(callStatusAgentReport);
